...
# ---------------- LOAD DATA ----------------
# Using 'dataset_split/train' for Train/Val split
...
# Test set from 'dataset_split/test' folder
...
# ---------------- PIPELINE ----------------
# We apply CLAHE first, then Augmentation.

def augment_batch(x, y):
    return augmentation(x, training=True), y

# Apply CLAHE
# Use tf.data.AUTOTUNE for parallel processing
...
# Optimization
# LIMIT DATASET FOR FASTER TRAINING (User Request)
# The full dataset is very large (~80,000 images). We will use 20% of it for now.
print("⚠️ Using 20% of the dataset for faster training")
train_ds = train_ds.take(int(len(train_ds) * 0.2))
val_ds   = val_ds.take(int(len(val_ds) * 0.2))
# Keep test set full or reduce? Let's reduce for speed too.
test_ds  = test_ds.take(int(len(test_ds) * 0.2))
...
def build_model():
    ...
    # Using 'int' label mode from image_dataset_from_directory for 2 classes works with this.
    # Class 0: negative_IDC
    # Class 1: positive_IDC
    output = layers.Dense(1, activation="sigmoid")(x)
    
    model = models.Model(inputs=inputs, outputs=output, name="Breast_EfficientNet_CLAHE")
...
checkpoint = callbacks.ModelCheckpoint(
    "breast_cancer_efficientnet_clahe.keras",
    monitor="val_accuracy",
    save_best_only=True,
    mode="max",
    verbose=1
...
model.save("breast_cancer_efficientnet_clahe.keras")
print("\nModel saved to breast_cancer_efficientnet_clahe.keras")
...
# class_names are ['negative_IDC', 'positive_IDC']
# y_pred = 0 -> negative_IDC
# y_pred = 1 -> positive_IDC
...
def make_gradcam_heatmap(img_array, model, last_conv_layer_name, pred_index=None):
    ...
    with tf.GradientTape() as tape:
        ...
        if pred_index is None:
            pred_index = tf.argmax(preds[0]) # For binary, index 0 is scalar output? No, wait.
            # For Binary (sigmoid output), model.output is (1, 1).
            # We want gradient of this scalar output wrt last conv layer.
            # But wait, grad_model outputs [last_conv, preds].
            # preds shape is (1, 1).
            
        # For binary sigmoid output, we just take the output itself as the target score
        # regardless of class, because it represents probability of Positive class.
        # If we want attribution for Negative class, we'd look at (1-output).
...
# Test image (using 'img1.png' if exists, else first from test set)
img_path = "img1.png"
